...
const scrapping = async () => {
  try {
    ...
    posts.forEach((post) => {
      if (!allPosts.some((exist) => exist.title === post.title) && !allPosts.some((exist) => exist.date === post.date)) {
        labels.forEach(label=>{
          label.dict.forEach(dict=>{
            if(post.title.toLowerCase().includes(dict) || post.content.toLowerCase().includes(dict) ){
